...
import {
  ...
  getReleases, getSamples, attachSampleToTrack, detachSampleFromTrack
} from '../../api/api';
...
export default function AdminTracks() {
  ...
  const load = () =>
    Promise.all([getTracks(), getReleases(), getSamples()])
      .then(([t, r, s]) => { setTracks(t); setReleases(r); setSamples(s); })
  ...
  return (
    <AdminLayout>
      ...
      <form onSubmit={handleSubmit} className="bg-gray-800 rounded-xl p-5 mb-8 flex flex-col gap-3 max-w-xl">
        <h2 className="font-semibold">{editing !== null ? 'Edit track' : 'Add track'}</h2>
        <input className={inputClass} placeholder="Title *" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} required />
        <select className={inputClass} value={form.release_id} onChange={e => setForm({ ...form, release_id: e.target.value })}>
          <option value="">— Release —</option>
          {releases.map(r => <option key={r.id} value={r.id}>{r.title}{r.artist ? ` – ${r.artist}` : ''}</option>)}
        </select>
            ...
            <div key={t.id} className="bg-gray-800 rounded-lg px-5 py-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  {t.release_title && <span className="text-gray-400 text-sm ml-3">{t.release_title}</span>}
                </div>
                <div className="flex gap-3">
                  <button onClick={() => { setAttachTrackId(t.id); setAttachSampleId(''); }} className="text-green-400 hover:text-green-300 text-sm transition-colors">+ Sample</button>
                  <button onClick={() => handleEdit(t)} className="text-blue-400 hover:text-blue-300 text-sm transition-colors">Edit</button>
                  <button onClick={() => handleDelete(t.id)} className="text-red-400 hover:text-red-300 text-sm transition-colors">Delete</button>
                </div>
              </div>

              {attachTrackId === t.id && (
                <form onSubmit={handleAttach} className="flex gap-2 mt-2">
                  <select className="bg-gray-700 text-white rounded-lg px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-blue-500 flex-1" value={attachSampleId} onChange={e => setAttachSampleId(e.target.value)} required>
                    <option value="">— Select sample —</option>
                    {samples.map(s => <option key={s.id} value={s.id}>{s.name}{s.source_title ? ` (${s.source_title})` : ''}</option>)}
                  </select>
                  <button type="submit" className="bg-green-600 hover:bg-green-500 text-white text-sm rounded-lg px-3 py-1.5 transition-colors">Attach</button>
                  <button type="button" onClick={() => setAttachTrackId(null)} className="text-gray-400 hover:text-white text-sm px-2 transition-colors">✕</button>
                </form>
              )}
              ...
              {t.samples && t.samples.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2">
                  {t.samples.map((s: any) => (
                    <span key={s.id} className="flex items-center gap-1 bg-gray-700 rounded-full px-3 py-0.5 text-xs text-gray-300">
                      {s.name}
                      <button onClick={() => handleDetach(t.id, s.id)} className="text-gray-500 hover:text-red-400 ml-1 transition-colors">✕</button>
                    </span>
                  ))}
                </div>
              )}
