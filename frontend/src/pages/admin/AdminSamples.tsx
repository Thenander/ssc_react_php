...
import {
  ...
  getSources, getTracks, getTypesByCategory,
  attachSampleToTrack, detachSampleFromTrack
} from '../../api/api';
...
export default function AdminSamples() {
  ...
  const load = () =>
    Promise.all([getSamples(), getSources(), getTracks(), getTypesByCategory('sample')])
      .then(([sa, so, tr, ty]) => { setSamples(sa); setSources(so); setTracks(tr); setTypes(ty); })
  ...
  return (
    <AdminLayout>
      ...
      <form onSubmit={handleSubmit} className="bg-gray-800 rounded-xl p-5 mb-8 flex flex-col gap-3 max-w-xl">
        <h2 className="font-semibold">{editing !== null ? 'Edit sample' : 'Add sample'}</h2>
        <input className={inputClass} placeholder="Name *" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required />
        <select className={inputClass} value={form.source_id} onChange={e => setForm({ ...form, source_id: e.target.value })}>
            ...
            <div key={s.id} className="bg-gray-800 rounded-lg px-5 py-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  {s.type_text && <span className="text-gray-400 text-xs ml-2">{s.type_text}</span>}
                  {s.source_title && <span className="text-gray-400 text-sm ml-3">← {s.source_title}</span>}
                </div>
                <div className="flex gap-3">
                  <button onClick={() => { setAttachSampleId(s.id); setAttachTrackId(''); }} className="text-green-400 hover:text-green-300 text-sm transition-colors">+ Track</button>
              ...
              {attachSampleId === s.id && (
                <form onSubmit={handleAttach} className="flex gap-2 mt-2">
                  <select className="bg-gray-700 text-white rounded-lg px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-blue-500 flex-1" value={attachTrackId} onChange={e => setAttachTrackId(e.target.value)} required>
                    <option value="">— Select track —</option>
                    {tracks.map(t => <option key={t.id} value={t.id}>{t.title}{t.release_title ? ` (${t.release_title})` : ''}</option>)}
                  </select>
                  ...
                  <button type="button" onClick={() => setAttachSampleId(null)} className="text-gray-400 hover:text-white text-sm px-2 transition-colors">✕</button>
                </form>
              )}
              ...
              {s.tracks && s.tracks.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2">
                  {s.tracks.map((t: any) => (
                    <span key={t.id} className="flex items-center gap-1 bg-gray-700 rounded-full px-3 py-0.5 text-xs text-gray-300">
                      {t.title}
                      <button onClick={() => handleDetach(s.id, t.id)} className="text-gray-500 hover:text-red-400 ml-1 transition-colors">✕</button>
                    </span>
                  ))}
                </div>
              )}
