...
export default function AdminTypes() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        editing !== null ? "Failed to update type." : "Failed to create type.",
      );
    }
  };
    ...
    <AdminLayout>
      ...
      >
        <h2 className="font-semibold">
          {editing !== null ? "Edit type" : "Add type"}
        </h2>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-6">
          {CATEGORIES.map((cat) => (
            <div key={cat}>
              <h2 className="text-lg font-semibold mb-2 capitalize text-gray-300">
                {cat}
              </h2>
              {grouped[cat].length === 0 ? (
                ...
                <div className="flex flex-col gap-1">
                  {grouped[cat].map((t) => (
                    <div
                      key={t.id}
                      className="bg-gray-800 rounded-lg px-5 py-2 flex items-center justify-between"
                    >
